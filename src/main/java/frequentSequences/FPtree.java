...
public class FPtree extends TotalSupportTree {
	...
	System.out.println("numCopls " + numCols);
	System.out.println("support " + support);
	System.out.println("confidence " + confidence);
	System.out.println("dataArray " + dataArray);
	System.out.println("minSupport " + minSupport);
	System.out.println("numOneItemSets " + numOneItemSets);
	System.out.println("conversionArray " + conversionArray);
	System.out.println("reconversionArray " + reconversionArray);*/
    ...
    /* Number of output metghods:
    1. Prefix subtree (Command line and GUI version).
    2. FP Tree (Command line and GUI version).
    3. FP Tree statistics (Command line and GUI version).
    4. Abcestor trail (Command line version only for diagnostic testing).
    5. Other diagnostic output methods.       */

    /*------------------------*/
    /* 1. OUTPUT HEADER TABLE */
    /*------------------------*/
    ...
    public void outputItemPrefixSubtree() {
        ...
        System.out.println("Format: Header N, then list (N) itemSet:count");
        for(int index=1;index<headerTable.length;index++) {
    ...
    /** Commences process of outputting the prefix sub tree to the screen,
    starting at header table (GUI version).
    ...
    /*public void outputItemPrefixSubtree(JTextArea textArea) {
	    ...
	    textArea.append("Header = " +
	                   reconvertItem(headerTable[index].itemName) + "\n");
	    flag = outputItemPrefixTree(textArea,headerTable[index].nodeLink);
	    if (flag!=1) textArea.append("\n");;
	    }
	textArea.append("\n");
	}*/

    /** Commences process of outputting a local prefix sub tree to the screen.
    @param tableRef the reference to the local header table (GUI version).
    @param textArea the given instance of the <TT>JTextArea</TT> class. */
    ...
    /*private void outputItemPrefixSubtree(JTextArea textArea,
	    ...
	    textArea.append("Header = " +
	                       reconvertItem(tableRef[index].itemName) + "\n");
	    flag = outputItemPrefixTree(textArea,tableRef[index].nodeLink);
	    if (flag!=1) textArea.append("\n");
	    }
	textArea.append("\n");
	}     */

    /** Outputs the given prefix sub tree (GUI version).
    @param textArea the given instance of the <TT>JTextArea</TT> class.
    @param ref the reference to the given branch.
    @return a counter representing the current "node number" (used in
    output). */
    ...
    /** Commences process of outputting FP-tree to screen (GUI version).
    @param textArea the given instance of the <TT>JTextArea</TT> class. */
   ...
   /* private void outputFPtreeNode1(JTextArea textArea) {
	...
	// Otherwise process
        ...
        for (int index=0;index<ref.length;index++) {
	    textArea.append("(" + nodeID + (index+1) + ") ");
	    outputItemPrefixSubtreeNode(textArea,ref[index].node);
	    outputFPtreeNode2(textArea,ref[index].childRefs,
                                                       nodeID+(index+1)+".");
	    }
	}*/
    ...
    /** Outputs the given prefix sub tree node (GUI version).
    @param textArea the given instance of the <TT>JTextArea</TT> class.
    @param ref the reference to the given node. */
   ...
   /*public void outputFPtreeStats(JTextArea textArea) {
    ...
    /*--------------------------------------------*/
    /* 5. VARIOUS OTHER DIAGNOSTIC OUTPUT METHODS */
    /*--------------------------------------------*/
