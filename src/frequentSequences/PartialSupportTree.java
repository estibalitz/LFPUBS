...
public class PartialSupportTree extends TotalSupportTree {
    ...
    distinction is that this method causes input data to be readfrom a P-tree
    ...
    public void contCreateTtree() {
	// Create Top level of T-tree (First pass of dataset). Defined in
	// in TotlaSupportTree class. Defined in in TotlaSupportTree class.
	createTtreeTopLevel();
    ...
    Only need to process those branches represented by the P-tree node label
    (not the complete itemset represented by the node, i.e. the union of the 
    node label and all its parent labels). Method includes bounds on which
    branches should be processed: for example the lower bound is typically
    attribute 1 as this attribute cannot have a sub-branch, the upper bound is
    typivally the lasty attribute plus 1 in the top level array of T-tree nodes 
    in the T-tree.
    ...
    values for appropriate nodes in T-tree at a given levelin the branch and 
