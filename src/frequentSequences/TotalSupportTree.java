    ...
    */	
    ...
    private void testRuleSupConfFwork(short[] antecedent, short[] consequent,
        ...
        if (confidenceForAR >= confidence)
                                   insertRuleintoRulelist(antecedent,
		     				  consequent,confidenceForAR);
	}
	...
	// If lift greater than 1 add to rule list.
	if (lift > 1.0) insertRuleintoRulelist(antecedent,consequent,lift);
        }
    ...
    /** Sets the levels in the T-tree field to the givenm value.
    @param mumLevels the given number of levels in the T tree value. */
    ...
    @return the number of support sets. */
    ...
    in connection with the DOC algorithm.</P>
    ...
    private TtreeNode[] copyTtree2(TtreeNode[] oldTtreeLevel) {
    ...
    (9)  Output duration (GUI version only)
    (10)  Output serialization array
    (11) Output serialized T-tree array (with support values)
    (12) Output serialized T-tree array (no support values)
    (13) Output serialized T-tree for level N */
	...
	for (int index=1;index<linkRef.length;index++) {
	    if (linkRef[index] != null) {
	        textArea.append("[" + number + num + "] {" + itemSetSofar +
	    		index);
    ...
    public void outputFrequentSets(ArrayList<simpleSequence> tempSetsOfActions) {
	int number = 1;
	...
	for (short index=1; index <= numOneItemSets; index++) {
	    ...
	    if (startTtreeRef[index] !=null) {
	        if (startTtreeRef[index].support >= minSupport) {
	        	//these sequences are made up of only one action
	        	simpleSequence tempSimpleSequence = new simpleSequence();
	            short[] itemSetSofar = new short[1];
	            itemSetSofar[0] = index;
	            //set length
	            tempSimpleSequence.setLength(1);
	            //set counter
	            tempSimpleSequence.setCounter(startTtreeRef[index].support);
	            //set sequence
	            tempSimpleSequence.setSequence(outputItemSet(itemSetSofar));
	            //set instances
	            
	            //add to tempSetsOfActions
	            tempSetsOfActions.add(tempSimpleSequence);
	            
	            //System.out.print("[" + number + "]");
                    //outputItemSet(itemSetSofar);
		    //System.out.println("= " + startTtreeRef[index].support);
	            number = outputFrequentSets(number+1,itemSetSofar,
		    		       index,startTtreeRef[index].childRef, tempSetsOfActions);
		    }
		}
	    }
    ...
    private int outputFrequentSets(int number, short[] itemSetSofar, int size,
    							TtreeNode[] linkRef, ArrayList<simpleSequence> tempSetsOfActions) {
	...
	for (short index=1; index < size; index++) {
	    if (linkRef[index] != null) {
	        if (linkRef[index].support >= minSupport) {
	        	//these sequences are made up of more than one action
	        	simpleSequence tempSimpleSequence = new simpleSequence();
	            short[] newItemSet = realloc2(itemSetSofar,index);
	            //set counter
	            tempSimpleSequence.setCounter(linkRef[index].support);
	            //set sequence
	            tempSimpleSequence.setSequence(outputItemSet(newItemSet));
	            //set length
	            tempSimpleSequence.setLength(tempSimpleSequence.getSequence().size());
	            //set instances
		    ...
		    //System.out.println("= " + linkRef[index].support);
	            number = outputFrequentSets(number + 1,newItemSet,index,
		    			           linkRef[index].childRef, tempSetsOfActions);
	            }
		}
	    }
    ...
    /* ------------------------------------------------- */
    /* 11. OUTPUT SERIALIZED T-TREE (WITH SUPPORT VALUES) */
    /* ------------------------------------------------- */
